...
function useReactVorm(definition, { validateOnChange = false, validateOnBlur = false } = {}) {
  ...
  function onSubmit(e) {
    ...
    if (e && e._reactName === 'onSubmit') { // eslint-disable-line
      e.preventDefault();
    } else {
  ...
  return {
    ...
    // Try not to use this, only for debugging
    respToErrors: (errorResponse) => respToErrors(errorResponse),

    setErrors: (errorResponse) => setErrors(respToErrors(errorResponse)),
    setValues,
    setTouched,
